            ...
            try:
                for (x, y, w, h) in faces:
                    ...
                    if current_frame % 10 != 0:
                        ...
                        for frame in sessions[session_id]:
                            print("I'm FOR LOOP")
                            #print("session_id len(sessions[session_id])", session_id, len(sessions[session_id]))
                            if len(analysis_per_session[session_id]) == 0:
                                analysis_per_session[session_id].HV = 0
                                analysis_per_session[session_id].LV = 0
                                analysis_per_session[session_id].HA = 0
                                analysis_per_session[session_id].HL = 0
                            ...
                            if frame.emotion == 'sad':
                                analysis_per_session[session_id].LA += 1
                                analysis_per_session[session_id].LV += 1
                            elif frame.emotion == 'happy':
                                analysis_per_session[session_id].HA += 1
                                analysis_per_session[session_id].HV += 1
                            elif frame.emotion == 'neutral':
                                analysis_per_session[session_id].LA += 1
                                analysis_per_session[session_id].HV += 1
                        
                        analysis_per_session[session_id].LA = round(analysis_per_session[session_id].LA / 60, 3)
                        analysis_per_session[session_id].HA = round(analysis_per_session[session_id].HA / 60, 3)

                        analysis_per_session[session_id].LV = round(analysis_per_session[session_id].LV / 60, 3)
                        analysis_per_session[session_id].HV = round(analysis_per_session[session_id].HV / 60, 3)
